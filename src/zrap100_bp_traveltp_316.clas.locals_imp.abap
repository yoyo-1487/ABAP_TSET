CLASS lhc_zrap100_r_traveltp_316 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    CONSTANTS:
...
CLASS lhc_zrap100_r_traveltp_316 IMPLEMENTATION.
...
**********************************************************************
  METHOD earlynumbering_create.
    DATA:
      entity           TYPE STRUCTURE FOR CREATE ZRAP100_R_TravelTP_316,
    ...
    "Remove the entries with an existing Travel ID( 刪除 Travel ID 有值的 )
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap100_atrav316 FIELDS MAX( travel_id ) AS travelID INTO @travel_id_max.
      "Get max travel ID from draft table
      SELECT SINGLE FROM zrap100_dtrav316 FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
...
**********************************************************************
  METHOD setStatusToOpen.
    ...
    READ ENTITIES OF ZRAP100_R_TravelTP_316 IN LOCAL MODE
     ENTITY Travel
    ...
    MODIFY ENTITIES OF ZRAP100_R_TravelTP_316 IN LOCAL MODE
      ENTITY Travel
        UPDATE SET FIELDS
...
**********************************************************************
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE ZRAP100_R_TravelTP_316.
...
**********************************************************************
  METHOD copyTravel.
    DATA:
       travels       TYPE TABLE FOR CREATE zrap100_r_traveltp_316\\travel.
